...
class LocationPickerState extends State<LocationPicker> {
  ...
  void autoCompleteSearch(String place) {
    ...
    var endpoint =
        "https://maps.googleapis.com/maps/api/place/autocomplete/json?key=${widget.apiKey}&input={$place}$regionParam&sessiontoken=$sessionToken&language=${widget.language}";
...
//  }
  ...
  void getNearbyPlaces(LatLng latLng) {
    LocationUtils.getAppHeaders().then((headers) {
      var endpoint =
          "https://maps.googleapis.com/maps/api/place/nearbysearch/json?key=${widget.apiKey}&location=${latLng.latitude},${latLng.longitude}&radius=150&language=${widget.language}";
  ...
  Future reverseGeocodeLatLng(LatLng latLng) async {
    final endpoint =
        "https://maps.googleapis.com/maps/api/geocode/json?latlng=${latLng.latitude},${latLng.longitude}&key=${widget.apiKey}&language=${widget.language}";
...
//          responseJson['results'][0]['address_components'][1]['short_name'];
      ...
      setState(() {
        ...
        if (addressComponents != null) {
          for (var item in addressComponents) {
